...
- [Verified] This pass is limited to repo-level investigation of the direct-product clone set in `C:\Users\Godsgrace\Desktop\codez\Oleriq-competitor-lab\repos\direct-products\`.
...
- [Verified] The durable per-repo notes are stored under `C:\Users\Godsgrace\Desktop\codez\Oleriq-competitor-lab\notes\<slug>\analysis.md`.
...
- [Inference] Oleriq's current strength is still the end-user document workflow: readable export, direct-file handling, and document batch conversion.
- [Inference] Oleriq's current weakness is agent-native breadth: explicit crawl controls, structured extraction contracts, renderer selection, MCP or CLI surfaces, and browser-session tooling.
...
- [Inference] It is a stronger ergonomics reference than a UI reference for Oleriq.
...
- [Verified] Its overlap with Oleriq is strongest on structured extraction across web and local documents, not on product surface or storage design.
...
- [Verified] It is not the strongest direct benchmark for Oleriq's core extraction job because the inspected CLI explicitly marks `extract` as not yet implemented.
...
- [Inference] Oleriq currently exposes one strong user-facing web app, but not the same multi-surface reach.
...
- [Verified] Oleriq already has job state for `/batch`.
- [Inference] Oleriq is closer to this pattern than it looks, but it does not expose the same breadth of crawl or extract job types yet.
...
- [Inference] Oleriq's current export set is strong for documents, but narrow for machine-facing extraction contracts.
...
- [Verified] Oleriq already has a strong user-facing export workflow with `pdf`, `txt`, `md`, and `docx`.
- [Verified] Oleriq already handles direct-file URLs and document batch uploads on `/batch`.
- [Verified] Oleriq already has queue-backed batch status, session-bound downloads, and durable storage options.

### Current Gaps

- [Inference] Oleriq lacks a narrow MCP surface or CLI surface that would let agents use it as a tool instead of only as a website.
- [Inference] Oleriq lacks an explicit structured-extraction contract comparable to schema-first or JSON-first competitors.
- [Inference] Oleriq lacks visible renderer or engine controls, renderer-decision metadata, and browser-escalation transparency.
- [Inference] Oleriq lacks crawl, map, and search surfaces that several competitors now bundle into the same product family.
- [Inference] Oleriq lacks low-friction developer onboarding primitives such as prefix-based usage, one-command installer setup, or self-host-first docs.

### Gaps That Matter Less Right Now

- [Inference] Full browser task automation like `browser-use` is not the best immediate target because it expands Oleriq into a different product category.
...
- [Inference] `Structured extraction mode`: Add a machine-facing extraction contract on top of Oleriq's current extract flow, instead of only export-oriented outputs.
...
- [Inference] `Async extract or crawl jobs`: Extend Oleriq's existing job model beyond document batch where it materially improves large or slow extraction tasks.

### Medium Value

- [Inference] `Selector and token-budget controls`: `jina-reader` shows clear value here, especially for machine consumers.
- [Inference] `Local-first installer or self-host docs`: `webclaw` is the strongest packaging reference if Oleriq chooses to court agent users directly.

### Lower Value For Now

- [Inference] `Prompt-graph orchestration`: `scrapegraphai` is useful inspiration, but the dependency and LLM-coupling cost is high relative to Oleriq's current surface.
- [Inference] `Full crawl infrastructure`: `apify-crawlee` is strong engineering reference, but it can pull Oleriq into unnecessary crawler-generalization too early.
...
- [Verified] `C:\Users\Godsgrace\Desktop\codez\Oleriq-competitor-lab\notes\firecrawl\analysis.md`
