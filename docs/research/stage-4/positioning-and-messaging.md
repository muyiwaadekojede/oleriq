...
- [Inference] `Clean output you can actually use.`
- [Inference] This angle fits the current homepage promise and the Stage 4 ranking best.

### Secondary Angle

- [Inference] `Know what was extracted, what was preserved, and where the limits are.`
- [Inference] This angle addresses trust without pretending every page or file is easy.

### Batch Angle

- [Inference] `Batch conversion with honest job truth, not silent failures.`
- [Inference] This fits the existing `/batch` surface and the ranking around job control and diagnostics.
...
- [Inference] The first-run story should not only promise speed.
- [Inference] It should promise `fast first success plus honest second-step behavior`.
- [Inference] The key emotional transition is from `this worked once` to `I trust this enough to keep using it`.

## Messaging Risks To Avoid

- [Inference] Do not imply full hostile-site or authenticated scraping capability unless the product truly supports it.
- [Inference] Do not imply that every output is equally faithful across all formats and layouts.
- [Inference] Do not lead with agent-platform language if the product still wins mainly on human-facing export and conversion.

## Recommended Near-Term Message Stack

- [Inference] Homepage: `Paste any URL. Get a clean, exportable document you can trust.`
- [Inference] Support copy: `See when content was reduced, incomplete, or file-based before you export.`
- [Inference] Batch copy: `Convert pages and documents in bulk with clearer status and cleaner outputs.`
