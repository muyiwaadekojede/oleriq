# Oleriq Opportunity Brief

## Executive View

- [Verified] The ranked Stage 3 shortlist says Oleriq's strongest next move is to become more trustworthy at the job it already claims to do.
...
- [Inference] The market gap closest to Oleriq is not lack of capabilities alone. It is the gap between a job being marked successful and a user trusting the output enough to use it.

## What Oleriq Should Optimize For

- [Inference] `Clean output that survives scrutiny`
  Oleriq already promises clean, exportable documents. The strongest opportunity is to make that promise more defensible with structure-preservation signals, explicit degradation warnings, and better document-quality truth.

- [Inference] `Fast first success without second-step disappointment`
  Competitors win demos and lose trust when the first real edge case appears. Oleriq should optimize for the transition from first extraction to first non-trivial extraction.

- [Inference] `Honest extraction state`
  Users repeatedly react badly to false-success states, empty-success states, and unclear renderer paths. Oleriq should make outcome truth a visible product surface.
...
- [Inference] Oleriq should treat tables, headings, lists, code blocks, OCR-heavy pages, and document hierarchy as first-class quality boundaries.
...
- [Inference] Oleriq should reduce the drop between first success and first edge-case failure.
...
- [Inference] Why now: the market repeatedly rewards easy first success, but Oleriq can differentiate by staying trustworthy on the second use.
...
- [Inference] Oleriq should not pretend all URLs are equal.
...
- [Inference] Oleriq should distinguish `processed`, `usable`, `partial`, and `failed` instead of flattening them into one success shape.
...
- [Inference] Oleriq does not need a full agent replay stack to benefit from better debugging.
...
- [Inference] If Oleriq expands packaging or usage visibility, it should favor legibility over cleverness.
...
- [Inference] Oleriq already has a meaningful `/batch` surface, so deterministic filenames, better per-item state, rerun clarity, and more honest queue behavior are close-range opportunities.
...
- [Inference] The practical move is not generic “AI extraction.” It is targeted structured output where Oleriq can still explain what was preserved or lost.

## Later Or Avoid For Now

### Agent Packaging And MCP

- [Inference] This is strategically interesting only if Oleriq decides to become more tool-facing or agent-facing.
...
## What Oleriq Should Not Copy Blindly
...
- [Inference] Oleriq should position itself as the trustworthy extraction and export layer for people who need usable output, not just successful requests.
- [Inference] If Oleriq wins structural fidelity, trust signals, and second-use confidence, it can add broader surfaces later from a stronger base.
